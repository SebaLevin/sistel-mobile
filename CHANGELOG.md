...
- [lib/data.js](lib/data.js) + [screens/SearchScreen.js](screens/SearchScreen.js): **la búsqueda no encontraba por código interno (`Codigo`, ej. `SALTA.173`)**. La app solo consultaba `Concepto` (nombre) y `CodBar` (EAN, vía heurística de dígitos), nunca la columna `Codigo`. Ahora una única función `searchProducts(term)` busca el término en las 3 columnas (`Concepto`, `Codigo`, `CodBar`) en paralelo y mergea por `numint` sin duplicados — la caja manual, el scanner de cámara y el lector físico usan todos esa búsqueda. Reemplaza a `searchProductsByName`/`searchProductsByCode` y la heurística `isBarcode`. Validado contra prod: `SALTA.173`→Codigo, `7790895000232`→CodBar, nombre→Concepto.
...
- **Migración a Expo SDK 56** (desde SDK 52). React Native `0.76.9` → `0.85.3`, React/React-DOM `18.3.1` → `19.2.3`, Hermes v1 como engine por defecto. Paquetes alineados vía `npx expo install --fix`: `expo` `~52` → `^56`, `expo-camera` `~16.0.18` → `~56.0.7`, `expo-secure-store` `~14.0.1` → `~56.0.4`, `expo-status-bar` `~2.0.1` → `~56.0.4`, `expo-system-ui` `~4.0.9` → `~56.0.5`, `expo-build-properties` `~0.13.3` → `~56.0.16`, `@react-native-async-storage/async-storage` `1.23.1` → `2.2.0`, `react-native-gesture-handler` `~2.20.2` → `~2.31.1`, `react-native-reanimated` `3.16.3` → `4.3.1`, `react-native-safe-area-context` `4.12.0` → `~5.7.0`, `react-native-screens` `4.4.0` → `4.25.2`, `react-native-web` `0.19.13` → `^0.21.2`. Devdeps: `@babel/core` → `^7.29.0`, `eslint-config-expo` → `~56.0.4`.
- Agregado `react-native-worklets@0.8.3`: Reanimated 4 lo requiere como peer dependency (el plugin de Babel se separó del paquete; `babel-preset-expo` lo configura solo).
- [app.json](app.json): `expo-status-bar` agregado como config plugin (en SDK 56 dejó de ser solo runtime). Lo hizo `expo install` automáticamente.
- Bundle de validación (`expo export --platform android`) OK bajo RN 0.85 / React 19. **Falta validar el build nativo en EAS y probar en dispositivo** (módulos nativos bajo New Arch, fuentes de `react-native-vector-icons`).
